...
class DoublyLL:
    ...
    def find_pair(self, target):
        ...
        curr = self.head
        result = []
        while curr is not None: ## Go till last Node
            curr_val = curr.val
            rem = target - curr_val
            curr_itr = curr.next
            while curr_itr is not None:
                if curr_itr.val == rem:
                    if [curr_val, rem] not in result:
                        result.append([curr_val, rem])
                curr_itr = curr_itr.next
            curr = curr.next

        return result
    
    def find_pair_sorted(self, target):
        if self.head is None and self.head.next is None:
            return []
        curr = self.head
        ...
        # 1. Find the tail node (right pointer)
        while right.next is not None:
            right = right.next
        ## This approach is similar to binary search
        ...
        while left != right and right.next != left:
            ...
            if curr_sum == target:
                ...
                left = left.next  ## curr_sum is small so we will have to move towards right to get bigger value for addition
            else:
                right = right.prev
    ...
    def find_pair_unsorted(self, target):
        if self.head is None or self.head.next is None:
            return []
        ...
        while curr is not None:
            ...
            if rem in seen:
                # To avoid duplicates like [rem, curr.val] and [curr.val, rem]
                pair = [rem, curr.val]
    ...
    def remove_dupli_sorted_doubly_ll(self):
        # Edge case: 0 or 1 nodes means no duplicates possible
        if self.head is None or self.head.next is None:
            return self.head
        ...
        while curr.next is not None:
            if curr.val == curr.next.val:
                # We found a duplicate! Let's track the node AFTER the duplicate
                next_unique = curr.next.next

                # Link curr forward to bypass the duplicate
                curr.next = next_unique

                # If there is a node after the duplicate, point its back-pointer to curr
                if next_unique is not None:
                    next_unique.prev = curr

                # NOTICE: We DO NOT move curr forward here! 
                # We stay on 'curr' just in case the NEXT node is ALSO a duplicate (e.g., 1 -> 1 -> 1)
            else:
                curr = curr.next
